        ...
        herdJob = plugin.launch {
            ...
            for (cow in cows) {
                ...
                if (System.currentTimeMillis() - lastMoveTime[cow]!! >= stuckTimeMs) {
                    if (cow.isValid) {
                        cow.world.spawnParticle(Particle.EXPLOSION_NORMAL, cow.location.add(0.0, 1.0, 0.0), 1)
                        cow.remove()
                    }
                    continue
                }
                if (isOnGround(cow)) {
                    ...
                    cowDirections[cow] = cowDirections[cow]!!.setY((-1.0).coerceAtLeast(cowDirections[cow]!!.y - 0.03))
                }
                if (isOnGround(cow) && System.currentTimeMillis() - lastMoveTime[cow]!! >= forceMoveTimeMs) {
                ...
                for (plr in player.world.players) {
                    ...
                    if (plr.persistentDataContainer.get(TaggedKeyBool("can_take_damage")) == false) {
                        continue
                    }
                    ...
                    val brawlDamageEvent = BrawlDamageEvent(plr, player, cowDamage, BrawlDamageType.SPECIAL)
                    brawlDamageEvent.knockbackMultiplier = knockback
                    brawlDamageEvent.callEvent()
                    cow.world.spawnParticle(Particle.EXPLOSION_LARGE, cow.location.add(0.0, 1.0, 0.0), 1)
                    cow.world.playSound(cow.location, Sound.ENTITY_COW_HURT, 1.5f, 0.75f)
                    cow.world.playSound(cow.location, Sound.ENTITY_ZOMBIE_ATTACK_WOODEN_DOOR, 0.75f, 0.8f)
                }
            }
