...
class MilkSpiralAbility(
    ...
    override fun doAbility() {
        ...
        val spiralLocation = player.location.add(Vector(0.0, 1.0, 0.0)).add(direction.clone().multiply(2.0))
        var doVelocity = true

        spiralJob = plugin.launch {
            while (true) {
                ...
                if (player.isSneaking || currentTime - spiralStartTimeMs >= velocityDurationMs) {
                    doVelocity = false
                }
                ...
                while (totalAddedDistance < totalDistance) {
                    val firstParticle = oldLocation.clone().add(getCirclePoint(firstCircle, secondCircle, theta, spiralRadius))
                    val secondParticle = oldLocation.clone().add(getCirclePoint(firstCircle, secondCircle, theta + Math.PI, spiralRadius))

                    if (first) {
                        firstParticle.world.playSound(firstParticle, Sound.ENTITY_GENERIC_SPLASH, 0.2f, 0.75f)
                        secondParticle.world.playSound(secondParticle, Sound.ENTITY_GENERIC_SPLASH, 0.2f, 0.75f)
                        first = false
                    }

                    firstParticle.world.spawnParticle(Particle.FIREWORKS_SPARK, firstParticle, 1)
                    secondParticle.world.spawnParticle(Particle.FIREWORKS_SPARK, secondParticle, 1)
                ...
                player.world.players.forEach { plr ->
                    ...
                    if (lastDamageTime[plr] != null && currentTime - lastDamageTime[plr]!! < damageCooldownMs) {
                        return@forEach
                    }
                    ...
                    if (plr.location.add(0.0, 1.5, 0.0).distance(spiralLocation) >= hitboxRadius) {
                        return@forEach
                    }
                    ...
                    val brawlDamageEvent = BrawlDamageEvent(plr, player, damage, BrawlDamageType.PROJECTILE)
    ...
    private fun getCirclePoint(firstCircle: Vector, secondCircle: Vector, theta: Double, radius: Double) : Vector{
