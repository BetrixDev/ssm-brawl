 ...
 */
val KtorApiLogger =
    createClientPlugin("KtorApiLogger", ::KtorApiLoggerConfig) {
...
private fun captureHeadersFromBuilder(
    headersBuilder: HeadersBuilder,
    sanitizedHeaders: Set<String>,
): Map<String, String> {
    val headers = headersBuilder.build()
    return headers.entries().associate { (key, values) ->
        val value =
            if (sanitizedHeaders.any { it.equals(key, ignoreCase = true) }) {
                "***REDACTED***"
            } else {
                values.joinToString(", ")
            }
    ...
    val logData = buildJsonObject {
        ...
        putJsonObject("request") {
            ...
            putJsonObject("headers") {
                requestHeaders.forEach { (key, value) -> put(key, JsonPrimitive(value)) }
            }
        }
        ...
        if (!success || errorMessage != null) {
            ...
            putJsonObject("error") { put("has_error", JsonPrimitive(false)) }
        }
