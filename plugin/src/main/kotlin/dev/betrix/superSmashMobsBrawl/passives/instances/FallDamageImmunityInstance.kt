...
class FallDamageImmunityInstance(definition: PassiveDefinition, player: Player) :
    ...
    private fun createLandingExplosion(fallDistance: Float) {
        ...
        player.sendMessage(
            "§6Landing explosion! §7(Fall distance: §e${fallDistance.toInt()} blocks§7)"
        )
    }
    ...
    private fun damageNearbyEnemies(location: org.bukkit.Location, explosionScale: Float) {
        ...
        nearbyPlayers.forEach { target ->
            ...
            if (distance <= scaledRadius) {
                ...
                val finalDamage =
                    explosionDamage * explosionScale * damageMultiplier.coerceAtLeast(0.3)
                ...
                player.sendMessage(
                    "§6Landing explosion hit ${target.name} for §c${finalDamage.toInt()} §6damage!"
                )
            }
        }
    }
