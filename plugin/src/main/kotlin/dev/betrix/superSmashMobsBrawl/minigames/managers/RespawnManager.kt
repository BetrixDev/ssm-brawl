...
class DefaultRespawnManager(private val minigame: BrawlMinigame) : 
    ...
    override fun initialize(minigame: BrawlMinigame) {
        ...
        listeners.add(
            BrawlDeathEvent.listen(this) {
                // Only handle deaths for players in this minigame
                if (minigame.allPlayers().none { it.isOnline && it.player?.uniqueId == player.uniqueId }) {
                    return@listen
                }
                
                // Fire analytics event
                PlayerDeathAnalyticsEvent(player, DeathReason.Damage, minigame).callEvent()
    ...
    override suspend fun handlePlayerDeath(player: Player) {
        ...
        if (minigame.minigameDef.respawnDelaySeconds != null) {
            ...
            if (world != null) {
                (minigame.teleportationHandler as? ITeleportationHandler)?.teleportPlayerToSpectatorArea(player)
            }
