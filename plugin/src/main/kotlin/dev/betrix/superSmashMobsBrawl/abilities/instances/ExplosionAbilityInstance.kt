...
class ExplosionAbilityInstance(definition: AbilityDefinition, player: Player) :
    ...
    private fun startCharging() {
        ...
        delay(1) { checkMovement(startLocation) }
    }
    ...
    private fun explode() {
        ...
        // Sound effects
        explosionLocation.world?.playSound(
            explosionLocation,
        ...
        nearbyPlayers?.forEach { target ->
            ...
            if (distance <= explosionRadius) {
                ...
                target.addPotionEffect(
                    PotionEffect(PotionEffectType.BLINDNESS, 10, 0, false, false)
                )

                player.sendMessage(
                    "§6Explosion hit ${target.name} for §c${finalDamage.toInt()} §6damage!"
                )
            }
        }
    }
    ...
    private fun respawnPlayer(explosionLocation: org.bukkit.Location) {
        ...
        val respawnLocation =
            findSafeRespawnLocation(explosionLocation) ?: explosionLocation.add(0.0, 5.0, 0.0)
        ...
        // Respawn effects
        respawnLocation.world?.spawnParticle(
        ...
        for (attempts in 0..20) {
            ...
            for (y in world.maxHeight downTo world.minHeight) {
                ...
                if (block.type.isSolid) {
                    val safeLocation =
                        org.bukkit.Location(world, testLocation.x, y + 1.1, testLocation.z)
                    if (
                        safeLocation.block.type.isAir &&
                            safeLocation.add(0.0, 1.0, 0.0).block.type.isAir
                    ) {
                        return safeLocation
                    }
                    break
