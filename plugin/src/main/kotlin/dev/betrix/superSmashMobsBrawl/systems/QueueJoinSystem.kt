...
class QueueJoinSystem(private val lang: LangService = inject()): IteratingSystem(
    family { all(PlayerTryQueueComponent, PlayerComponent) }
) {
    ...
    override fun onTickEntity(entity: Entity) {
        ...
        if (queuedEntities.contains(entity)) {
            player.sendErrorMessage(lang.t("messages.queue.join.alreadyInQueue") {
        ...
        if (partiedEntities.contains(entity)) {
            ...
            if (partyEntity == null) {
                ...
                TODO("check for party leader, if leader see if party can queue for minigame, if not leader, don't let queue at all")
            }
        }

        TODO("Handle adding player to queue. Make sure to remove the PlayerTryQueueComponent component")
    }
}