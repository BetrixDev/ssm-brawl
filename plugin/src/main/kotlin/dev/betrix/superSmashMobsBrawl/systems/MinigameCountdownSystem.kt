...
class MinigameCountdownSystem(
	...
	override fun onTick() {
		starting.forEach { entity ->
			...
			if (minigame.remainingCountdownSeconds == null) {
				minigame.remainingCountdownSeconds = max(0, minigame.minigame.startingCountdownSeconds ?: 0)
			...
			if (secondsLeft > 0) {
				...
				// hand off to start system which teleports/assigns kits if teleport-before wasn't used
				minigame.state = MinigameState.STARTING // keep in STARTING; start system will push to ONGOING
	...
	private fun broadcastCountdown(entity: Entity, secondsLeft: Int) {
		...
		minigame.playerEntities.forEach { pEntity ->
			...
			if (minigame.minigame.startSoundEachSecond) {
				player.playSound(
					Sound.sound(Key.key("minecraft:block.note_block.hat"), Sound.Source.MASTER, 1f, 1.8f),
				)
			}
		}
	}
	...
	private fun broadcastGo(entity: Entity) {
		...
		minigame.playerEntities.forEach { pEntity ->
			...
			player.showTitle(title)
			player.playSound(
				Sound.sound(Key.key("minecraft:block.note_block.bell"), Sound.Source.MASTER, 1f, 1.2f),
			)
		}
	}
}