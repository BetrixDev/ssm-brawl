...
object WorldUtils {
    ...
    /**
     * Gets the actual world name that might be prefixed, checking both prefixed and non-prefixed versions
     */
    ...
    /**
     * Copies a world from ./worlds directory and loads it with a custom name (automatically prefixed)
    ...
    /**
     * Copies a world from ./worlds directory and loads it with a custom name (async, automatically prefixed)
     * @param sourceWorldName Name of the world folder in ./worlds directory
     * @param customWorldName Custom name for the new world (will be prefixed with "ssmbworld_")
        ...
        Bukkit.getScheduler().runTaskAsynchronously(
            ...
            Runnable {
                ...
                Bukkit.getScheduler().runTask(
                    Bukkit.getPluginManager().plugins.first(),
                    Runnable {
                        val loadResult = copyResult.fold(
                            onSuccess = { worldName ->
                                runCatching {
     ...
     * Handles both prefixed and non-prefixed world names
     * @param worldName Name of the world to delete (with or without prefix)
     * @return Result indicating success or failure
     */
    fun deleteWorld(worldName: String): Result<Unit> {
        return runCatching {
            val actualWorldName = getActualWorldName(worldName)
                ?: error("World '$worldName' not found (checked both prefixed and non-prefixed versions)")
            ...
            if (world != null) {
                ...
                val mainWorld = Bukkit.getWorlds().firstOrNull { it != world }
    ...
    /**
     * Checks if a world name is available (not loaded and no folder exists)
     * Checks both the base name and prefixed version
    ...
    private fun deleteWorldFolder(worldPath: Path): Result<Unit> {
        return runCatching {
            Files.walkFileTree(worldPath, object : SimpleFileVisitor<Path>() {
                override fun visitFile(file: Path, attrs: BasicFileAttributes): FileVisitResult {
                    Files.delete(file)
                    return FileVisitResult.CONTINUE
                }
